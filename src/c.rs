...
enum Statement<'a> {
    ...
    Declaration(Type<'a>, Symbol<'a>, Option<Expression<'a>>),
    Block(Block<'a>),
    For {
        name: Symbol<'a>,
        from: Expression<'a>,
    ...
    pub fn to_c(&self) -> Program {
        ...
        for (_, func) in &self.functions {
            ...
            for stmt in &func.body.statements {
                let (mut stmts, expr) = stmt.to_c_statements(&mut namer);
...
impl<'a> ast::Expression<'a> {
    fn to_c_statements(&self, namer: &mut Namer) -> (Vec<Statement<'a>>, Option<Expression<'a>>) {
        ...
        match &self.kind {
            ...
            Ek::Block(block) => {
                let (block, new_variable) =
                    block.to_c_statements(namer.gen_name("block_name"), namer);
                match new_variable {
            ...
            Ek::FunctionCall {
                ...
                for arg in args {
                    let (mut arg_pre, arg) = arg.to_c_statements(namer);
            ...
            Ek::If {
                ...
                let else_block = match else_block {
                    Some(else_block) => {
                        ...
                        let (else_block, _) = else_block.to_c_statements(name.clone(), namer);
                        Some(else_block)
                    }
                    None => None,
                };
            ...
            Ek::Field(expr, field) => {
                let (pre, expr) = expr.to_c_statements(namer);
            ...
            Ek::For {
                ...
                let (mut pre, from) = from.to_c_statements(namer);
                let (mut to_pre, to) = to.to_c_statements(namer);
...
impl<'a> ast::Block<'a> {
    fn to_c_statements(
        ...
        for stmt in &self.statements {
            let (mut stmts, expr) = stmt.to_c_statements(namer);
            statements.append(&mut stmts);
            if let Some(expr) = expr {
                statements.push(Statement::Expression(expr));
            }
    ...
    pub fn print(&self) -> String {
        ...
        for s in &self.structs {
            let fields = s
                .fields
                .iter()
                .map(|(name, typ)| format!("{} {};", typ.print(), name))
                .join("\n");
...
impl<'a> Block<'a> {
    fn print(&self) -> String {
        let body = self.statements.iter().map(|stmt| stmt.print()).join(";\n") + ";\n";
        format!("{{\n{}\n}}", indent(&body))
    }
}
    ...
    fn print(&self) -> String {
        match self {
            Statement::Declaration(typ, name, expr) => match expr {
                Some(expr) => format!("{} {} = {}", typ.print(), name, expr.print()),
                None => format!("{} {}", typ.print(), name),
            },
            Statement::If {
                condition,
                if_block,
                ...
                out.push(format!("if ({})", condition.print()));
                out.push(if_block.print());
                if let Some(else_block) = else_block {
                    out.push(format!("else {}", else_block.print()));
                }

                out.join(" ")
            }
            Statement::Expression(expr) => expr.print(),
            Statement::Return(expr) => format!("return {}", expr.print()),
            Statement::Block(block) => block.print(),
            Statement::For {
                name,
                from,
                to,
...
impl<'a> Expression<'a> {
    fn print_with_precedence(&self, precedence: usize) -> String {
        match self {
            Expression::Operator(lhs, op, rhs) => {
                let lhs = lhs.print_with_precedence(op.precedence() + 1);
                let rhs = rhs.print_with_precedence(op.precedence() + 1);
            ...
            Expression::LValue(l) => l.print(),
            Expression::Literal(l) => l.print(),
            Expression::FunctionCall(name, args) => format!(
                "{}({})",
                name,
                args.iter().map(|arg| arg.print()).join(", ")
            ),
        }
    }
    fn print(&self) -> String {
        self.print_with_precedence(0)
    }
}
...
impl<'a> LValue<'a> {
    fn print(&self) -> String {
        match self {
            ...
            LValue::Field(expr, field) => format!("{}.{}", expr.print(), field),
            x => unimplemented!("{:?}", x),
        }
    }
}
